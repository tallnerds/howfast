...
const generateMarkdown = (metrics, runs) => {
  ...
  for (let run = 1; run <= runs; run++) {
    const cols = [
      ...
      _.get(metrics, `firstContentfulPaint.actual.numericValues.${run - 1}`),
      _.get(metrics, `largestContentfulPaint.actual.numericValues.${run - 1}`),
      _.get(metrics, `speedIndex.actual.numericValues.${run - 1}`),
      _.get(metrics, `cumulativeLayoutShift.actual.numericValues.${run - 1}`),
      _.get(metrics, `serverResponseTime.actual.numericValues.${run - 1}`),
    ];
